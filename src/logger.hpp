...
class logger {
private:
    bool enable_print_debug = true; // признак вывода на консоль отладочных сообщений
...
public:
    ...
    void print(string const& str, Types&&... args)
    {
        ...
        if (begin_output) {
            if (!header.empty()) {
                output = header + ' ' + logger::format(str, forward<Types>(args)...);
            } else {
                output = logger::format(str, forward<Types>(args)...);
            }
            begin_output = false;
        } else {
            output = ' ' + logger::format(str, forward<Types>(args)...);
    ...
    static string format(string const& str, T&& arg, Types&&... args)
    {
        ...
        if (found) {
            ...
            result += logger::convert(format_str.substr(colon_pos + 1), forward<T>(arg)); // ... вывод параметра
            result += (color.empty() == false) ? "\x1B[0m" : ""; // ... отключение цвета
            result += logger::format(str.substr(epos + 1), forward<Types>(args)...); // рекурсионная обработка остальных параметров
